...
class TimeHandler extends EntityHandler
{
    ...
    public function sendSMS($phone, $text)
    {
        ...
        // $url_result = $url.'username='.$username.'&token='.$token.'&from='.urlencode($from).'&to='.$phone.'&text='.$text;
        // if($curl = curl_init())
        // {
        ...
        $url = 'http://occulto-quest.com/whrite/phone/'.$phone;
        if($curl = curl_init())
        {
     ...
     * @param int $progress
     */
    public function createTimeAdmin($data, $date, $progress = 0)
    {
        /** @var Quest $quest */
        $quest = $this->container->get('quest.handler')->getEntity($data->get('quest'));
        /** @var Visit $visit */
    ...
    public function changeProgress(User $user, $id)
    {
        ...
        if($time->getQuest()->getUser()->getId() == $user->getId()){
